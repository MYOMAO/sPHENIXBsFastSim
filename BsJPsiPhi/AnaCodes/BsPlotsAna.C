...
void BsPlotsAna(){
	...
	TString TrackptCut = Form("(epPt > %f && emPt > %f && kpPt > %f && kmPt > %f)",TrackptCutValue,TrackptCutValue,TrackptCutValue,TrackptCutValue);
	...
	TString BsSigCut = "(IsSigEP*IsSigEM*IsSigKP*IsSigKM==1)";
	TString BsSelCut = "(abs(KKMass - 1.019461) < 0.1 && abs(eeMass - 3.096900) < 0.10)";

	TString BsDecayCut = "(IsSigEP>2 && IsSigEM>2 && IsSigKP>2 && IsSigKM>2)";
	TString BsCombCut = "(IsSigEP==0 && IsSigEM==0 && IsSigKP==0 && IsSigKM==0)";



	if(UseTrackptCut == 1){

		BsSigCut = Form("(IsSigEP*IsSigEM*IsSigKP*IsSigKM==1) && %s",TrackptCut.Data());
		BsSelCut = Form("(abs(KKMass - 1.019461) < 0.1 && abs(eeMass - 3.096900) < 0.10) && %s",TrackptCut.Data());
	...
	TTree * BsJPsiPhiKKee = (TTree *) fin->Get("BsJPsiPhiKKee");
...
*/
	...
	for(int i = 0; i < NVar; i++){
		...
		BsJPsiPhiKKee->Project(AllName.Data(),VarName[i].Data());
		BsHisAll->Write();
		...
		BsJPsiPhiKKee->Project(CombName.Data(),VarName[i].Data(),BsCombCut.Data());
		...
		BsJPsiPhiKKee->Project(DecayName.Data(),VarName[i].Data(),BsDecayCut.Data());
		...
		BsJPsiPhiKKee->Project(SigName.Data(),VarName[i].Data(),BsSigCut.Data());
		...
		BsJPsiPhiKKee->Project(SelName.Data(),VarName[i].Data(),BsSelCut.Data());
